...
export function VSLPerformance({ data }: VSLPerformanceProps) {
  const milestones = [
    { 
      label: "5% watched", 
      count: data.vsl5Percent, 
      dropOff: data.dropOff5Percent,
      color: "bg-purple-500" 
    },
    { 
      label: "25% watched", 
      count: data.vsl25Percent, 
      dropOff: data.dropOff25Percent,
      color: "bg-blue-500" 
    },
    { 
      label: "75% watched", 
    ...
    { 
      label: "95% watched", 
      count: data.vsl95Percent, 
      dropOff: data.dropOff95Percent,
      color: "bg-cyan-500" 
    },
  ];

  const maxCount = data.totalLeads || 1;
                ...
                <div className="flex items-center gap-4">
                  ...
                  {milestone.dropOff > 0 && (
                    <span className="text-xs text-red-600">
                      {milestone.dropOff.toFixed(1)}% dropped off
                    </span>
                  )}
                </div>
              </div>
