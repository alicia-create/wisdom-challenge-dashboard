-- ============================================================================
-- UNIFIED DASHBOARD METRICS SQL FUNCTION v10
-- ============================================================================
-- Changes from v9:
-- 1. Added leads and purchases per Meta campaign type from ad_performance table
-- 2. Meta Performance table now shows: Spend, Purchases, CPP, Sales Rate, Leads, CPL, Lead Rate, Clicks, CPC, CTR, CPM
-- ============================================================================
...
DECLARE
    ...
    -- Meta campaign breakdown variables (with leads and purchases)
...
BEGIN
    ...
    -- Organic leads (31daywisdomchallenge.com or other)
    ...
    WHERE o.created_at >= p_start_date
      ...
      AND oi.product_id = 1 -- Backstage Pass only
      AND EXISTS (
          SELECT 1 FROM analytics_events ae
          WHERE ae.contact_id = c.id
            AND ae.name = 'form.submitted'
    ...
    -- ========================================================================
    -- SECTION 3: REVENUE (from specific products)
    -- ========================================================================
    ...
    WHERE o.created_at >= p_start_date
      AND o.created_at < p_end_date + INTERVAL '1 day'
      AND oi.product_id IN (1, 2, 4) -- Backstage Pass, Express Shipping, Extra Wisdom Journal
      AND EXISTS (
          SELECT 1 FROM analytics_events ae
          WHERE ae.contact_id = c.id
            AND ae.name = 'form.submitted'
    ...
    WHERE o.created_at >= p_start_date
      AND o.created_at < p_end_date + INTERVAL '1 day'
      AND oi.product_id = 8 -- Kingdom Seekers Trial
      AND EXISTS (
          SELECT 1 FROM analytics_events ae
          WHERE ae.contact_id = c.id
            AND ae.name = 'form.submitted'
    ...
    -- ========================================================================
    -- SECTION 5: MANYCHAT & BOT ALERTS
    -- ========================================================================
    
    -- ManyChat Connected (has manychat_id)
    SELECT COUNT(DISTINCT c.id)
    ...
    -- ========================================================================
    -- SECTION 7: META PERFORMANCE - TOTAL (with leads and purchases)
    -- ========================================================================
    ...
    INTO v_meta_spend, v_meta_clicks, v_meta_landing_page_views, v_meta_impressions, v_meta_leads, v_meta_purchases
    FROM ad_performance
    WHERE date >= p_start_date AND date <= p_end_date AND platform ILIKE 'meta';
    
    -- ========================================================================
    -- SECTION 7B: META CAMPAIGN BREAKDOWN BY TYPE (with leads and purchases)
    -- ========================================================================
    ...
    WHERE ae.timestamp >= p_start_date
      AND ae.timestamp < p_end_date + INTERVAL '1 day'
      AND ae.name = 'vidalytics.milestone'
    ...
    v_result := jsonb_build_object(
        ...
        'metaPerformance', jsonb_build_object(
            ...
            'connectRate', CASE WHEN v_meta_clicks > 0 THEN ROUND((v_meta_landing_page_views::NUMERIC / v_meta_clicks * 100)::NUMERIC, 2) ELSE 0 END
        ),
        ...
        'vslPerformance', jsonb_build_object(
            ...
            'vsl25To75PercentRate', CASE WHEN v_vsl_25_percent > 0 THEN ROUND((v_vsl_75_percent::NUMERIC / v_vsl_25_percent * 100)::NUMERIC, 2) ELSE 0 END,
            'vsl75To95PercentRate', CASE WHEN v_vsl_75_percent > 0 THEN ROUND((v_vsl_95_percent::NUMERIC / v_vsl_75_percent * 100)::NUMERIC, 2) ELSE 0 END,
            ...
            'overallRetention75', CASE WHEN v_total_leads > 0 THEN ROUND((v_vsl_75_percent::NUMERIC / v_total_leads * 100)::NUMERIC, 2) ELSE 0 END,
            ...
            'dropOff25To75', CASE WHEN v_vsl_25_percent > 0 THEN ROUND(((v_vsl_25_percent - v_vsl_75_percent)::NUMERIC / v_vsl_25_percent * 100)::NUMERIC, 2) ELSE 0 END,
            'dropOff75To95', CASE WHEN v_vsl_75_percent > 0 THEN ROUND(((v_vsl_75_percent - v_vsl_95_percent)::NUMERIC / v_vsl_75_percent * 100)::NUMERIC, 2) ELSE 0 END
        ),
