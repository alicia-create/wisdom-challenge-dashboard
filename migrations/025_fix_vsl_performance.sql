-- ============================================================================
-- Migration 025: Fix VSL Performance Tracking
-- Change from 'video.watched' to 'vidalytics.view_video'
-- Use value ILIKE '%X%' to match watch percentages
-- ============================================================================
...
BEGIN
    ...
    -- CTE 7: VSL metrics (video watch percentages) - FIXED
    vsl_metrics AS (
        SELECT
            COUNT(DISTINCT contact_id) FILTER (WHERE value ILIKE '%5%') as vsl_5_percent,
            COUNT(DISTINCT contact_id) FILTER (WHERE value ILIKE '%25%') as vsl_25_percent,
            COUNT(DISTINCT contact_id) FILTER (WHERE value ILIKE '%50%') as vsl_50_percent,
            COUNT(DISTINCT contact_id) FILTER (WHERE value ILIKE '%95%') as vsl_95_percent
        FROM analytics_events
        WHERE name = 'vidalytics.view_video'
          AND timestamp >= p_start_date::timestamp
          AND timestamp < v_next_day_str::timestamp
    )
...
-- Add comment
COMMENT ON FUNCTION get_dashboard_metrics IS 'v25 - FIX: VSL Performance now uses vidalytics.view_video events with value ILIKE pattern matching. Email clicks set to 0 (no keap.email.clicked events available).';
