-- ============================================================================
-- UNIFIED DASHBOARD METRICS SQL FUNCTION v9
-- ============================================================================
-- Changes from v8:
-- 1. Added Meta campaign breakdown by type: Sales, Leads, Retargeting, Content, Other
-- ============================================================================
...
DECLARE
    ...
    -- Meta campaign breakdown variables
...
BEGIN
    ...
    -- ========================================================================
    -- SECTION 7: META PERFORMANCE - TOTAL
    -- ========================================================================
    ...
    -- ========================================================================
    -- SECTION 7B: META CAMPAIGN BREAKDOWN BY TYPE
    -- ========================================================================
    ...
    SELECT 
        ...
        COALESCE(SUM(link_clicks), 0),
        COALESCE(SUM(impressions), 0)
    INTO v_meta_sales_spend, v_meta_sales_clicks, v_meta_sales_impressions
    FROM ad_performance
    WHERE date >= p_start_date AND date <= p_end_date 
    ...
    SELECT 
        ...
        COALESCE(SUM(link_clicks), 0),
        COALESCE(SUM(impressions), 0)
    INTO v_meta_leads_spend, v_meta_leads_clicks, v_meta_leads_impressions
    FROM ad_performance
    WHERE date >= p_start_date AND date <= p_end_date 
      AND platform ILIKE 'meta'
    ...
    SELECT 
        ...
        COALESCE(SUM(link_clicks), 0),
        COALESCE(SUM(impressions), 0)
    INTO v_meta_retargeting_spend, v_meta_retargeting_clicks, v_meta_retargeting_impressions
    FROM ad_performance
    WHERE date >= p_start_date AND date <= p_end_date 
      AND platform ILIKE 'meta'
    ...
    SELECT 
        ...
        COALESCE(SUM(link_clicks), 0),
        COALESCE(SUM(impressions), 0)
    INTO v_meta_content_spend, v_meta_content_clicks, v_meta_content_impressions
    FROM ad_performance
    WHERE date >= p_start_date AND date <= p_end_date 
      AND platform ILIKE 'meta'
    ...
    SELECT 
        ...
        COALESCE(SUM(link_clicks), 0),
        COALESCE(SUM(impressions), 0)
    INTO v_meta_other_spend, v_meta_other_clicks, v_meta_other_impressions
    FROM ad_performance
    WHERE date >= p_start_date AND date <= p_end_date 
      AND platform ILIKE 'meta'
      AND campaign_name NOT ILIKE '%SALES%'
    ...
    -- ========================================================================
    -- SECTION 9: TOTAL REVENUE (products 1, 2, 4 only)
    -- ========================================================================
    ...
    v_result := jsonb_build_object(
        ...
        -- Meta Campaign Breakdown by Type
        'metaCampaignBreakdown', jsonb_build_object(
            'sales', jsonb_build_object(
                'spend', v_meta_sales_spend,
                'clicks', v_meta_sales_clicks,
                'impressions', v_meta_sales_impressions,
                'cpc', CASE WHEN v_meta_sales_clicks > 0 THEN ROUND(v_meta_sales_spend / v_meta_sales_clicks, 2) ELSE 0 END,
                'cpm', CASE WHEN v_meta_sales_impressions > 0 THEN ROUND((v_meta_sales_spend / v_meta_sales_impressions) * 1000, 2) ELSE 0 END,
                'ctr', CASE WHEN v_meta_sales_impressions > 0 THEN ROUND((v_meta_sales_clicks::NUMERIC / v_meta_sales_impressions) * 100, 2) ELSE 0 END
            ),
            'leads', jsonb_build_object(
                'spend', v_meta_leads_spend,
                'clicks', v_meta_leads_clicks,
                'impressions', v_meta_leads_impressions,
                'cpc', CASE WHEN v_meta_leads_clicks > 0 THEN ROUND(v_meta_leads_spend / v_meta_leads_clicks, 2) ELSE 0 END,
                'cpm', CASE WHEN v_meta_leads_impressions > 0 THEN ROUND((v_meta_leads_spend / v_meta_leads_impressions) * 1000, 2) ELSE 0 END,
                'ctr', CASE WHEN v_meta_leads_impressions > 0 THEN ROUND((v_meta_leads_clicks::NUMERIC / v_meta_leads_impressions) * 100, 2) ELSE 0 END
            ),
            'retargeting', jsonb_build_object(
                'spend', v_meta_retargeting_spend,
                'clicks', v_meta_retargeting_clicks,
                'impressions', v_meta_retargeting_impressions,
                'cpc', CASE WHEN v_meta_retargeting_clicks > 0 THEN ROUND(v_meta_retargeting_spend / v_meta_retargeting_clicks, 2) ELSE 0 END,
                'cpm', CASE WHEN v_meta_retargeting_impressions > 0 THEN ROUND((v_meta_retargeting_spend / v_meta_retargeting_impressions) * 1000, 2) ELSE 0 END,
                'ctr', CASE WHEN v_meta_retargeting_impressions > 0 THEN ROUND((v_meta_retargeting_clicks::NUMERIC / v_meta_retargeting_impressions) * 100, 2) ELSE 0 END
            ),
            'content', jsonb_build_object(
                'spend', v_meta_content_spend,
                'clicks', v_meta_content_clicks,
                'impressions', v_meta_content_impressions,
                'cpc', CASE WHEN v_meta_content_clicks > 0 THEN ROUND(v_meta_content_spend / v_meta_content_clicks, 2) ELSE 0 END,
                'cpm', CASE WHEN v_meta_content_impressions > 0 THEN ROUND((v_meta_content_spend / v_meta_content_impressions) * 1000, 2) ELSE 0 END,
                'ctr', CASE WHEN v_meta_content_impressions > 0 THEN ROUND((v_meta_content_clicks::NUMERIC / v_meta_content_impressions) * 100, 2) ELSE 0 END
            ),
            'other', jsonb_build_object(
                'spend', v_meta_other_spend,
                'clicks', v_meta_other_clicks,
                'impressions', v_meta_other_impressions,
                'cpc', CASE WHEN v_meta_other_clicks > 0 THEN ROUND(v_meta_other_spend / v_meta_other_clicks, 2) ELSE 0 END,
                'cpm', CASE WHEN v_meta_other_impressions > 0 THEN ROUND((v_meta_other_spend / v_meta_other_impressions) * 1000, 2) ELSE 0 END,
                'ctr', CASE WHEN v_meta_other_impressions > 0 THEN ROUND((v_meta_other_clicks::NUMERIC / v_meta_other_impressions) * 100, 2) ELSE 0 END
            )
        ),
