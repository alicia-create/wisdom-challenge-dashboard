-- ============================================================================
-- UNIFIED DASHBOARD METRICS SQL FUNCTION v7
-- ============================================================================
-- Changes from v6:
-- 1. CPL = Total Spend (LEADS/SALES campaigns) / Paid Ads Leads
-- 2. CPP = Total Spend (LEADS/SALES campaigns) / Paid Ads Wisdom+ Sales
-- 3. Wisdom+ Sales = Only "31DWC - 2601 - Backstage Pass" (product_id = 1)
-- 4. Total Revenue = Sum of products 1, 2, 4 only:
--    - 31DWC - 2601 - Backstage Pass (id=1)
--    - 31DWC - 2601 - Express Shipping Fee (id=2)
--    - 31DWC - 2601 - Extra Wisdom Journal (id=4)
-- ============================================================================
...
DECLARE
    ...
    -- Ad performance (TOTAL)
    v_total_spend NUMERIC;
    v_total_revenue NUMERIC;
    v_link_clicks INT;
    v_landing_page_views INT;
    -- Ad performance (LEADS/SALES campaigns only for CPL/CPP/ROAS)
...
BEGIN
    ...
    -- Total Wisdom+ Sales (only Backstage Pass, product_id = 1)
    ...
    WHERE o.created_at >= p_start_date
      ...
      AND oi.product_id = 1  -- 31DWC - 2601 - Backstage Pass
      AND EXISTS (
          SELECT 1 FROM analytics_events ae
          WHERE ae.contact_id = c.id
            AND ae.name = 'form.submitted'
    ...
    -- Paid Ads Wisdom+ Sales (product_id = 1)
    SELECT COUNT(DISTINCT o.id)
    ...
    -- Organic Wisdom+ Sales (product_id = 1)
    SELECT COUNT(DISTINCT o.id)
    ...
    -- ========================================================================
    -- SECTION 6B: AD PERFORMANCE - LEADS/SALES CAMPAIGNS ONLY (for CPL/CPP/ROAS)
    -- ========================================================================
    ...
    -- Wisdom Conversion = Total Wisdom Sales / Total Leads
    ...
    -- ManyChat rate is now based on leads, not kingdom seekers
    ...
    v_result := jsonb_build_object(
        ...
        'kpis', jsonb_build_object(
            ...
            -- ROAS = Revenue / LEADS/SALES spend
        ...
        'paidAdsFunnel', jsonb_build_object(
            ...
            -- ManyChat rate = manychat_id / leads (all leads have bot access)
        ...
        'vslPerformance', jsonb_build_object(
            ...
            -- VSL retention rates (view X% / leads)
            ...
            -- VSL drop-off rates (leads - view X%) / leads
