-- ============================================================================
-- Migration 031: Fix Wisdom+ and Journals Product ID Logic
-- - Wisdom+ Sales: COUNT orders with product_id = 1 in order_items
-- - Total Journals: Wisdom+ count + SUM(quantity) from product_id = 4
-- - Remove order_total >= $31 logic
-- ============================================================================
...
BEGIN
    ...
    wisdom_orders AS (
        SELECT 
            ...
            o.billing_status,
            o.created_at,
            -- Pre-calculate paid/organic flags
            CASE 
                WHEN o.funnel_name LIKE '%31daywisdom.com%' AND o.funnel_name NOT LIKE '%challenge%' THEN true 
                ELSE false 
            END as is_paid_funnel,
            CASE 
                WHEN o.funnel_name LIKE '%31daywisdomchallenge.com%' THEN true 
                ELSE false 
            END as is_organic_funnel
        FROM orders o
        ...
        SELECT
            ...
            COUNT(DISTINCT wo.id) FILTER (
                WHERE EXISTS (
                    ...
                    WHERE oi.order_id = wo.id AND oi.product_id = 1
                )
            ) as wisdom_sales,
            -- Wisdom+ Sales excluding $0 orders (for AOV calculation)
            COUNT(DISTINCT wo.id) FILTER (
                WHERE wo.order_total > 0
                AND EXISTS (
                    SELECT 1 FROM order_items oi 
                    WHERE oi.order_id = wo.id AND oi.product_id = 1
                )
            ) as wisdom_sales_non_zero,
            COUNT(DISTINCT wo.id) FILTER (
                WHERE wo.is_paid_funnel
                AND EXISTS (SELECT 1 FROM order_items oi WHERE oi.order_id = wo.id AND oi.product_id = 1)
            ) as paid_wisdom_sales,
            COUNT(DISTINCT wo.id) FILTER (
                WHERE wo.is_organic_funnel
                AND EXISTS (SELECT 1 FROM order_items oi WHERE oi.order_id = wo.id AND oi.product_id = 1)
            ...
            -- Extra Journals (product_id = 4, SUM quantity)
            COALESCE((
                SELECT SUM(oi.quantity)
                FROM order_items oi
                JOIN wisdom_orders wo2 ON oi.order_id = wo2.id
            ...
            COALESCE((
                ...
                FROM order_items oi
                JOIN wisdom_orders wo2 ON oi.order_id = wo2.id
                WHERE oi.product_id = 4 AND wo2.is_paid_funnel
            ), 0) as paid_extra_journals,
            COALESCE((
                SELECT SUM(oi.quantity)
                FROM order_items oi
                JOIN wisdom_orders wo2 ON oi.order_id = wo2.id
                WHERE oi.product_id = 4 AND wo2.is_organic_funnel
            ), 0) as organic_extra_journals,
            
            -- Extra Shipping
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.funnel_name ILIKE '%shipping%') as extra_shipping,
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.funnel_name ILIKE '%shipping%' AND wo.is_paid_funnel) as paid_extra_shipping,
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.funnel_name ILIKE '%shipping%' AND wo.is_organic_funnel) as organic_extra_shipping,
            
            -- Revenue
            SUM(wo.order_total) as total_revenue,
            SUM(wo.order_total) FILTER (WHERE wo.is_paid_funnel) as paid_revenue,
            SUM(wo.order_total) FILTER (WHERE wo.is_organic_funnel) as organic_revenue
        FROM wisdom_orders wo
    ),
    engagement_metrics AS (
        SELECT
            -- Bot alerts
            COUNT(DISTINCT CASE WHEN ae.name = 'manychat.add_tag' AND ae.value = 'gold.ntn.request_accepted' AND wl.is_paid THEN ae.contact_id END) as paid_bot_alerts,
            COUNT(DISTINCT CASE WHEN ae.name = 'manychat.add_tag' AND ae.value = 'gold.ntn.request_accepted' AND wl.is_organic AND NOT wl.is_paid THEN ae.contact_id END) as organic_bot_alerts,
            
            -- Welcome email clicks
            COUNT(DISTINCT CASE WHEN ae.name = 'keap.add_tag' AND ae.value ILIKE '%Clicked NTN In Email%' AND wl.is_paid THEN ae.contact_id END) as paid_welcome_clicks,
            COUNT(DISTINCT CASE WHEN ae.name = 'keap.add_tag' AND ae.value ILIKE '%Clicked NTN In Email%' AND wl.is_organic AND NOT wl.is_paid THEN ae.contact_id END) as organic_welcome_clicks
    ...
    SELECT jsonb_build_object(
        'kpis', jsonb_build_object(
            ...
            'aov', CASE WHEN ps.wisdom_sales_non_zero > 0 THEN ROUND((ps.total_revenue / ps.wisdom_sales_non_zero)::numeric, 2) ELSE 0 END,
            ...
            'totalJournals', ps.wisdom_sales + ps.extra_journals,
            'welcomeEmailClicks', COALESCE(em.paid_welcome_clicks, 0) + COALESCE(em.organic_welcome_clicks, 0),
            'cplAds', CASE WHEN lm.paid_leads > 0 THEN ROUND(((COALESCE(am.meta_leads_spend, 0) + COALESCE(am.meta_sales_spend, 0)) / lm.paid_leads)::numeric, 2) ELSE 0 END,
            'cppAds', CASE WHEN ps.paid_wisdom_sales > 0 THEN ROUND(((COALESCE(am.meta_leads_spend, 0) + COALESCE(am.meta_sales_spend, 0)) / ps.paid_wisdom_sales)::numeric, 2) ELSE 0 END,
            'trueCpl', CASE WHEN lm.total_leads > 0 THEN ROUND((COALESCE(am.total_spend, 0) / lm.total_leads)::numeric, 2) ELSE 0 END,
            'trueCpp', CASE WHEN ps.wisdom_sales > 0 THEN ROUND((COALESCE(am.total_spend, 0) / ps.wisdom_sales)::numeric, 2) ELSE 0 END
        ),
...
COMMENT ON FUNCTION get_dashboard_metrics IS 'v31 - Fixed Wisdom+ (product_id=1) and Journals (Wisdom+ + SUM quantity product_id=4) calculations';
