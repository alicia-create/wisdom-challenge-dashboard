-- ============================================================================
-- Migration 026: Create VSL Analytics Materialized View
-- Pre-calculate VSL watch percentages for better performance
-- Follows same pattern as wisdom_lead_classification
-- ============================================================================

-- Drop existing materialized view if exists
DROP MATERIALIZED VIEW IF EXISTS vsl_analytics CASCADE;

-- Create materialized view for VSL analytics
CREATE MATERIALIZED VIEW vsl_analytics AS
SELECT
    contact_id,
    DATE(timestamp) as watch_date,
    MAX(CASE WHEN value ILIKE '%5%' THEN 1 ELSE 0 END) as watched_5_percent,
    MAX(CASE WHEN value ILIKE '%25%' THEN 1 ELSE 0 END) as watched_25_percent,
    MAX(CASE WHEN value ILIKE '%50%' THEN 1 ELSE 0 END) as watched_50_percent,
    MAX(CASE WHEN value ILIKE '%95%' THEN 1 ELSE 0 END) as watched_95_percent
FROM analytics_events
WHERE name = 'vidalytics.view_video'
GROUP BY contact_id, DATE(timestamp);

-- Create indexes for better query performance
CREATE INDEX idx_vsl_analytics_watch_date ON vsl_analytics(watch_date);
CREATE INDEX idx_vsl_analytics_contact_id ON vsl_analytics(contact_id);
CREATE INDEX idx_vsl_analytics_95_percent ON vsl_analytics(watched_95_percent) WHERE watched_95_percent = 1;

-- Grant select permissions
GRANT SELECT ON vsl_analytics TO authenticated, anon;

-- Add comment
COMMENT ON MATERIALIZED VIEW vsl_analytics IS 'Pre-calculated VSL watch percentages per contact per day from vidalytics.view_video events';

-- ============================================================================
-- Update get_dashboard_metrics to use vsl_analytics
-- ============================================================================
...
BEGIN
    ...
    -- CTE 7: VSL metrics (using materialized view)
    vsl_metrics AS (
        SELECT
    ...
    SELECT jsonb_build_object(
        ...
        'vslPerformance', jsonb_build_object(
            ...
            'conversionRate', CASE WHEN vm.vsl_95_percent > 0 THEN (ps.wisdom_sales::numeric / vm.vsl_95_percent) * 100 ELSE 0 END
        )
    ) INTO v_result
...
-- Add comment
COMMENT ON FUNCTION get_dashboard_metrics IS 'v26 - Uses vsl_analytics materialized view for better performance. Email clicks set to 0 (no keap.email.clicked events available).';
