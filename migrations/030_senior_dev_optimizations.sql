-- ============================================================================
-- Migration 030: Senior Dev Optimizations
-- 1. Fix welcome email clicks (keap.add_tag with "Clicked NTN In Email")
-- 2. Remove query redundancy (use CASE WHEN for paid/organic splits)
-- 3. Add performance indexes
-- 4. Verify Kingdom Seekers in paid funnel
-- ============================================================================

-- Step 1: Add indexes for performance
CREATE INDEX IF NOT EXISTS idx_analytics_events_name_value ON analytics_events(name, value);
CREATE INDEX IF NOT EXISTS idx_analytics_events_timestamp ON analytics_events(timestamp);
CREATE INDEX IF NOT EXISTS idx_analytics_events_contact_timestamp ON analytics_events(contact_id, timestamp);
CREATE INDEX IF NOT EXISTS idx_orders_funnel_billing ON orders(funnel_name, billing_status);
CREATE INDEX IF NOT EXISTS idx_orders_created_funnel ON orders(created_at, funnel_name);
CREATE INDEX IF NOT EXISTS idx_order_items_order_product ON order_items(order_id, product_id);
CREATE INDEX IF NOT EXISTS idx_vsl_analytics_watch_date ON vsl_analytics(watch_date);
CREATE INDEX IF NOT EXISTS idx_vsl_analytics_contact_watch ON vsl_analytics(contact_id, watch_date);

-- Step 2: Recreate get_dashboard_metrics with optimizations
...
BEGIN
    ...
    wisdom_orders AS (
        SELECT 
            ...
            o.billing_status,
            o.created_at,
            -- Pre-calculate paid/organic flags to avoid redundant LIKE operations
            CASE 
    ...
    product_sales AS (
        SELECT
            ...
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.is_paid_funnel) as paid_wisdom_sales,
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.is_organic_funnel) as organic_wisdom_sales,
            ...
            -- Extra Journals
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.funnel_name ILIKE '%journal%') as extra_journals,
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.funnel_name ILIKE '%journal%' AND wo.is_paid_funnel) as paid_extra_journals,
            COUNT(DISTINCT wo.id) FILTER (WHERE wo.funnel_name ILIKE '%journal%' AND wo.is_organic_funnel) as organic_extra_journals,
...
COMMENT ON FUNCTION get_dashboard_metrics IS 'v30 - Senior dev optimizations: welcome email clicks fixed, query redundancy removed, performance indexes added';
